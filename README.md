...
设置入口：点击菜单栏“设置”，进入主程序内的设置页（不是弹出新窗口）。
...
3：最后标出目标点在屏幕上的位置（按下TAB、ESC后将鼠标指针移动到想打的地方并Alt + Left标记），工具会根据仰角表计算仰角。
...
- 测量流程进行中触发 Alt + Q，不会立刻退出。
- 进入 1.5 秒判定窗口：
	- 1.5 秒内仅触发 1 次 Alt + Q：退出流程。
	- 1.5 秒内触发多次 Alt + Q：判定为误触，不退出流程。
